...
def test_majority_substrates_metastable():
    ...
    metastable = [
        name for name, (gamma, _) in SUBSTRATE_GAMMA.items()
        if abs(gamma - 1.0) < 0.15
    ]
    assert len(metastable) >= 4, (
        f"Only {len(metastable)}/6 substrates metastable: {metastable}"
    )
...
def test_mean_gamma_near_unity():
    ...
    assert abs(mean_g - 1.0) < 0.15, (
        f"Mean γ={mean_g:.4f} outside metastable zone"
    )
