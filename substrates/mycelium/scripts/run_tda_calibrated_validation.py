...
def main() -> int:
    parser = argparse.ArgumentParser(description="TDA-calibrated zebrafish gamma validation")
    ...
    if args.real_data:
        ...
        report = validator.from_mat_directory(args.real_data, grid_size=args.grid, label_real=True)
    else:
        print(f"[SYNTHETIC PROXY] grid={args.grid} timepoints={args.timepoints}")
        ...
        report = validator.validate(wt_arrays, mut_arrays, tr_arrays, label_real=False)
