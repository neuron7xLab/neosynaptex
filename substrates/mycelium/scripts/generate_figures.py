...
def fig1_ccp_reproducibility():
    ...
    ax1.axhline(
        data["D_f_mean"], color="#1B5E20", ls="-", lw=1.5, label=f"mean={data['D_f_mean']:.3f}"
    )
    ax1.set_xlabel("Seed")
    ax1.set_ylabel("$D_f$")
    ...
    ax2.axhline(data["R_mean"], color="#1B5E20", ls="-", lw=1.5, label=f"mean={data['R_mean']:.3f}")
    ax2.set_xlabel("Seed")
    ax2.set_ylabel("$R$")
    ...
    fig.suptitle(
        f"CCP Reproducibility: {data['cognitive_fraction']:.0%} cognitive, CV={data['D_f_cv']:.2%}",
        fontsize=11,
        y=1.02,
    )
    plt.tight_layout()
