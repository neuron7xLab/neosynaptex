...
def _correlation_dimension(field: np.ndarray, n_points: int = 500, seed: int = 42) -> float:
    ...
    radii = np.logspace(np.log10(np.percentile(dists, 1)), np.log10(np.percentile(dists, 50)), 15)
...
def run() -> dict:
    ...
    for seed in seeds:
        ...
        print(
            f"  seed={seed:2d}  D_box={d_box:.4f}  D_corr={d_corr:.4f}  D_hurst={d_hurst:.4f}  spread={spread:.4f}"
        )
