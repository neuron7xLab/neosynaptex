...
def run() -> dict:
    ...
    for seed in seeds_mfn:
        ...
        mfn_results.append(
            {"seed": seed, "D_f": ccp["D_f"], "R": ccp["R"], "cognitive": ccp["cognitive"]}
        )
    ...
    others_have_negative = any(
        not substrates[k]["passes_ccp"] for k in ("fhn_excitable", "kuramoto")
    )
    # Also: D_f must be computable for all substrates (measurement validity)
    all_d_f_valid = all(
        all(1.0 <= r["D_f"] <= 2.5 for r in s["results"]) for s in substrates.values()
    )
    ...
    for name, data in substrates.items():
        print(
            f"  {name:20s}: {'PASS' if data['passes_ccp'] else 'FAIL'} ({data['cognitive_fraction']:.0%} cognitive)"
        )
