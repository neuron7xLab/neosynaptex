...
def main() -> int:
    ...
    parser.add_argument("--grid", type=int, default=128, help="Grid size for synthetic proxy")
    parser.add_argument("--timepoints", type=int, default=30, help="Number of timepoints")
    parser.add_argument(
        "--output",
        type=Path,
    ...
    if args.real_data:
        ...
        adapter = ZebrafishFieldAdapter(AdapterConfig(target_grid_size=args.grid))
        ...
        if mat_files:
            ...
            if len(mut_candidates) >= 2:
                tr_seqs = adapter.from_mat(mut_candidates[1], phenotype="transition")
        else:
            # Fallback: try NPZ
        ...
        print("[SYNTHETIC PROXY] McGuirl 2020 data not provided. Using synthetic proxy.")
        print(f"  Grid: {args.grid}x{args.grid}, Timepoints: {args.timepoints}, Seed: {args.seed}")
