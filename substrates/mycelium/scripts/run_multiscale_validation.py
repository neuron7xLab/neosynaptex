...
def _noise_series(n: int, grid: int, seed: int) -> list:
    ...
    return [np.random.default_rng(seed * 100 + t).random((grid, grid)) for t in range(n)]
...
def main() -> int:
    ...
    validator = MultiScaleValidator(n_bootstrap=args.bootstrap, verbose=args.verbose)
    ...
    if args.real_data:
        ...
        if not mat_files:
            ...
            if mut_file and mut_file != wt_file:
                ...
                mut_seqs = adapter.from_mat_composite(mut_file, phenotype="mutant")
                mut_fields = [s.field for s in mut_seqs]
            report = validator.validate(wt_fields, mut_fields, label_real=True)
    else:
