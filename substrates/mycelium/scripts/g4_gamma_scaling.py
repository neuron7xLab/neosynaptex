...
def _compute_gamma_robust(
    ...
    null = [float(np.polyfit(x, rng.permutation(y), 1)[0]) for _ in range(n_bootstrap)]
    p_value = max(float(np.mean(np.abs(null) >= abs(gamma_ts))), 1.0 / n_bootstrap)
...
def run() -> dict:
    ...
    print("\n--- Gate Check ---")
    print(
        f"  Mean gamma: {mean_gamma:.4f} (range [0.5, 2.5]): {'PASS' if gamma_in_range else 'FAIL'}"
    )
