...
class TestChoiceOperator:
    ...
    def test_deterministic_across_calls(self) -> None:
        ...
        r1 = choice_operator(
            candidates=[0, 1, 2], scores=scores, seq=seq, candidate_fields=fields, seed=42
        )
        ...
        r2 = choice_operator(
            candidates=[0, 1, 2], scores=scores, seq=seq2, candidate_fields=fields2, seed=42
        )
...
class TestChoiceOperatorIntegration:
    def test_works_with_simulated_field(self) -> None:
        ...
        rng = np.random.RandomState(42)
        fields = [seq.field + rng.normal(0, 0.001, seq.field.shape) for _ in range(3)]
