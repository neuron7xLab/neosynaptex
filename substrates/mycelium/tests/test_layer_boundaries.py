...
class TestNoCircularImports:
    ...
    def test_core_imports_without_errors(self) -> None:
        ...
        # Save module state, clear, reimport, then restore to avoid
        # poisoning downstream tests with duplicate class objects.
        saved_modules = {
            m: sys.modules[m]
            for m in list(sys.modules)
            if m.startswith("mycelium_fractal_net")
        }
        ...
        try:
            ...
            # Restore original module objects to prevent isinstance breakage
            for m in list(sys.modules):
                if m.startswith("mycelium_fractal_net"):
                    del sys.modules[m]
