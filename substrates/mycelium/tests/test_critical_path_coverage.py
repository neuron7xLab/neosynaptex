...
class TestGammaDiagnostic:
    def test_gamma_with_varied_seeds(self):
        ...
        seqs = [
            simulate_history(SimulationSpec(grid_size=16, steps=30, seed=i, alpha=0.10 + i * 0.02))
            for i in range(8)
        ]
        try:
    ...
    def test_gamma_diagnosis_categories(self):
        ...
        seqs = [simulate_history(SimulationSpec(grid_size=16, steps=20, seed=i)) for i in range(10)]
        try:
            result = gamma_diagnostic(seqs)
            assert isinstance(result, str)
        except (ValueError, ZeroDivisionError):
