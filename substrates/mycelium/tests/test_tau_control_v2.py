...
class TestTrajectoryDiscriminant:
    ...
    def test_high_uncertainty_forces_operational(self) -> None:
        ...
        kind, _prob, unc = td.classify(
            phi=1.5, phi_trend=0.1, failure_density=0.15, coherence=0.5, steps_in_bad_phase=5
        )
        if unc > 0.4:
    ...
    def test_existential_requires_evidence(self) -> None:
        ...
        kind, _, _ = td.classify(
            phi=0.3, phi_trend=0.0, failure_density=0.05, coherence=0.8, steps_in_bad_phase=1
        )
        assert kind == PressureKind.OPERATIONAL
...
class TestTransformationKL:
    def test_kl_rejection(self) -> None:
        ...
        ns = NormSpace(np.zeros(4), np.eye(4))
        meta = MetaRuleSpace()
        _, accepted = tp.transform(
            meta, phi=5.0, free_energy=0.1, norm=ns, norm_origin=ns, meta_origin=meta
        )
