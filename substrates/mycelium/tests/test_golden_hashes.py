...
def _round_floats(obj: object, ndigits: int = 10) -> object:
    """Round all floats to *ndigits* so hashes are stable across CPUs."""
...
def _desc_hash(desc: mfn.MorphologyDescriptor) -> str:
    return hashlib.sha256(
        json.dumps(_round_floats(desc.to_dict()), sort_keys=True, default=str).encode()
    ).hexdigest()[:16]
