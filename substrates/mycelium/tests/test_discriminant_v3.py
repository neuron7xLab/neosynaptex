    ...
    return [
        {
            ...
            "phi_trend": rng.normal(0, 0.1),
            "failure_density": rng.uniform(0, 0.1),
            "coherence": rng.uniform(0.6, 1.0),
...
def _synth_existential(n: int = 300, seed: int = 43) -> list[dict[str, float]]:
    ...
    return [
        {
            "phi": rng.uniform(2.0, 8.0),
            "phi_trend": rng.uniform(0.1, 1.0),
            "failure_density": rng.uniform(0.3, 0.9),
            "coherence": rng.uniform(0.0, 0.3),
...
def _synth_ambiguous(n: int = 100, seed: int = 44) -> list[dict[str, float]]:
    ...
    return [
        {
            "phi": rng.uniform(0.8, 2.5),
            "phi_trend": rng.normal(0.05, 0.15),
            "failure_density": rng.uniform(0.1, 0.3),
            "coherence": rng.uniform(0.3, 0.6),
...
class TestDiscriminantResult:
    ...
    def test_backward_compatible_classify(self) -> None:
        ...
        ns = NormSpace(np.zeros(4), np.eye(4))
        result = d.classify(
            phi=0.1, tau=3.0, x=np.zeros(4), norm=ns, phase_is_collapsing=False, coherence=0.9
        )
    ...
    def test_ambiguous_defaults_to_operational(self) -> None:
        ...
        for sample in ambiguous:
            kind, _prob, _unc = td.classify(
                sample["phi"],
                sample["phi_trend"],
                sample["failure_density"],
                sample["coherence"],
                int(sample["steps_in_bad"]),
            )
...
class TestHysteresis:
    ...
    def test_hysteresis_counter_decays(self) -> None:
        ...
        for _ in range(2):
            d.classify_detailed(
                phi=10.0, tau=3.0, x=np.zeros(4), norm=ns, phase_is_collapsing=False, coherence=0.9
            )

        # One operational — decays by 1 instead of reset
        d.classify_detailed(
            phi=0.0, tau=99.0, x=np.zeros(4), norm=ns, phase_is_collapsing=False, coherence=0.9
        )
