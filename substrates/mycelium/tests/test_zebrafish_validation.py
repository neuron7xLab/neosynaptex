...
class TestSyntheticProxy:
    def test_wt_generates_correct_n_timepoints(self, gen):
        cfg = SyntheticZebrafishConfig(n_timepoints=15, phenotype=ZebrafishPhenotype.WILD_TYPE)
        fields = gen.generate_sequence(cfg)
        assert len(fields) == 15

    def test_wt_field_values_in_unit_interval(self, gen):
        cfg = SyntheticZebrafishConfig(n_timepoints=5, phenotype=ZebrafishPhenotype.WILD_TYPE)
        fields = gen.generate_sequence(cfg)
        for f in fields:
            assert f.min() >= -0.01
            assert f.max() <= 1.01
...
class TestGammaValidator:
    ...
    def test_insufficient_sequences_returns_invalid(self, adapter):
        ...
        arrays = gen.generate_sequence(
            SyntheticZebrafishConfig(n_timepoints=2, phenotype=ZebrafishPhenotype.WILD_TYPE)
        )
