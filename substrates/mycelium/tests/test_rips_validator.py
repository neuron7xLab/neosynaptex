...
class TestRipsControlGenerator:
    ...
    def test_series_length(self, stripe_series):
        assert len(RipsControlGenerator(0).full_permutation_series(stripe_series)) == len(
            stripe_series
        )
...
class TestRipsMHLComputer:
    ...
    def test_stripe_lt_noise(self, stripe_cloud, noise_cloud):
        c = RipsMHLComputer()
        assert (
            c.compute(stripe_cloud, "s").median_lifetime
            < c.compute(noise_cloud, "n").median_lifetime
        )
    ...
    def test_label_real(self, stripe_cloud):
        ...
        assert r.label_real is True
        assert r.evidence_type == "real"
