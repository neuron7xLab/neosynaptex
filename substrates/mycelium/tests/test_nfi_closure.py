...
from mycelium_fractal_net.nfi.gamma_probe import GammaEmergenceProbe
...
def test_gamma_probe_accepts_only_contracts():
    ...
    assert "field_sequence" not in params
...
def test_gamma_probe_emergent_has_source():
    ...
    if report.label == "EMERGENT":
        assert report.mechanistic_source is not None, "EMERGENT report must have mechanistic_source"
...
def test_tau_control_isolation():
    ...
    if not tau_dir.exists():
        ...
        tau_dir = (
            pathlib.Path(__file__).resolve().parent.parent / "mycelium_fractal_net" / "tau_control"
        )
    if not tau_dir.exists():
    ...
    for f in tau_dir.rglob("*.py"):
        ...
        assert "GammaEmergence" not in src, (
            f"ISOLATION VIOLATED in {f.name}: references GammaEmergence"
        )
