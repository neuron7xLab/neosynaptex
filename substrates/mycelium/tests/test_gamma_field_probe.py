...
def test_gamma_organized_fields_positive():
    ...
    for t in range(T):
        phase = t * 0.05
        fields[t] = np.exp(-((x - np.sin(phase)) ** 2 + (y - np.cos(phase)) ** 2) / 2.0)
...
def test_verdict_organized():
    ...
    report = GammaFieldReport(substrate="x", gamma=2.0, r2=0.5, ci_low=1.5, ci_high=2.5, n_pairs=50)
...
def test_run_on_mfn_engine():
    ...
    probe = GammaFieldProbe(n_bootstrap=50, seed=42)
    real, ctrl = probe.run_on_engine(engine, steps=100, substrate_label="mfn_activator")
