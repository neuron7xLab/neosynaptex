...
class GNCState:
    ...
    @classmethod
    def from_levels(
        cls, levels: dict[str, float], context: dict[str, float] | None = None
    ) -> GNCState:
...
def step(state: GNCState) -> GNCState:
    ...
    theta_next = {
        t: float(np.clip(state.theta[t] + delta[t] + eps[i], 0.1, 0.9)) for i, t in enumerate(THETA)
    }
...
def get_omega() -> np.ndarray:
    ...
    return _OMEGA.copy()  # type: ignore[no-any-return]
