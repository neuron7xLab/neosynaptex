...
class NeuromodulatoryDigitalTwin:
    ...
    def validate(self) -> dict[str, Any]:
        ...
        for i in range(3, len(self.history)):
            ...
            mae = np.mean([abs(predicted.modulators[m] - actual.modulators[m]) for m in MODULATORS])
            errors.append(float(mae))

        # Variance of history
        all_levels = np.array([[s.modulators[m] for m in MODULATORS] for s in self.history])
    ...
    @classmethod
    def from_gnc_states(
        cls, states: list[GNCState], window: int = 10
    ) -> NeuromodulatoryDigitalTwin:
