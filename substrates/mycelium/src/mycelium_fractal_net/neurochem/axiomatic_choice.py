...
def check_activation_conditions(
    ...
    if prev_gnc_state is not None:
        delta_theta = float(
            np.mean([abs(gnc_state.theta[t] - prev_gnc_state.theta[t]) for t in THETA])
        )
...
class AxiomaticChoiceOperator:
    ...
    def select(
        ...
        if prev_state is not None:
            delta_theta = float(
                np.mean([abs(selected.theta[t] - prev_state.theta[t]) for t in THETA])
            )
            if delta_theta < 1e-6:
    ...
    def validate_axioms(
        ...
        if prev_state is not None:
            dt = float(np.mean([abs(selected.theta[t] - prev_state.theta[t]) for t in THETA]))
            a4 = dt > 1e-6
        else:
            a4 = True
