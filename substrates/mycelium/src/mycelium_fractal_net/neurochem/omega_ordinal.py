...
def compute_ordinal_dynamics(
    ...
    active = [
        ix for ix in omega.interactions if abs(state.modulators.get(ix.source, 0.5) - 0.5) > 0.15
    ]

    phase_risk = float(
        np.clip(
            len([ix for ix in omega.interactions if ix.rank == OrdinalRank.OMEGA_SQ]) * 0.3
            + (level.value / 3.0) * 0.7,
            0,
            1,
        )
    ...
    return {
        ...
        "active_interactions": [(ix.source, ix.target, ix.rank.label()) for ix in active],
