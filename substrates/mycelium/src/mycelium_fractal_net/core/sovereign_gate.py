...
class SovereignVerdict:
    ...
    def __str__(self) -> str:
        ...
        lines.append("╠" + "─" * w + "╣")
        lines.append(
            "║" + f"  {self.n_passed}/{self.n_passed + self.n_failed} lenses passed".ljust(w) + "║"
        )
...
class SovereignGate:
    ...
    def _lens_thermodynamic(self, field: np.ndarray) -> LensVerdict:
        ...
        try:
            ...
            if ep.sigma < -1e-8:
                return LensVerdict(
                    "thermodynamic", False, 0.2, f"σ={ep.sigma:.6f} < 0 (2nd law)", ep.sigma
                )
    ...
    def _lens_topological(self, field: np.ndarray, history: np.ndarray | None) -> LensVerdict:
        ...
        try:
            ...
            if history is not None and history.shape[0] >= 3:
                ...
                if drift > 20:
                    return LensVerdict(
                        "topological", False, 0.3, f"Topo drift={drift} (unstable)", float(drift)
                    )
    ...
    def _lens_transport(self, history: np.ndarray | None) -> LensVerdict:
        ...
        try:
            ...
            detail = (
                f"W₂: {w2_start:.4f}→{w2_mid:.4f} ({'converging' if converging else 'diverging'})"
            )
