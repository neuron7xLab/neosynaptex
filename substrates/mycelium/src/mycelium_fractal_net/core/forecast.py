...
def _roll_forward(
    ...
    if descriptor is None:
        descriptor = compute_morphology_descriptor(sequence)
    plasticity_index = float(descriptor.neuromodulation.get("plasticity_index", 0.0))  # type: ignore[attr-defined]
    connectivity_divergence = float(descriptor.connectivity.get("connectivity_divergence", 0.0))  # type: ignore[attr-defined]
...
def forecast_next(
    ...
    descriptor_shift = float(abs(last_step.D_box - descriptor.features.get("D_box", 0.0)))  # type: ignore[attr-defined]
    activity_shift = float(abs(last_step.f_active - descriptor.features.get("f_active", 0.0)))  # type: ignore[attr-defined]
