...
class LyapunovAnalyzer:
    ...
    def compute_jacobian_spectrum(
        ...
        if fn_name in JACOBIAN_REGISTRY or any(k in fn_name.lower() for k in JACOBIAN_REGISTRY):
...
class ThermodynamicKernel:
    ...
    def _build_report(
        ...
        if verdict == "unstable" or not drift_ok:
            gate_passed = False
            gate_message = (
                f"GATE CLOSED: verdict={verdict}, λ₁={lam1_max:.4f}, drift={mean_drift:.2e}"
            )
        ...
        return ThermodynamicStabilityReport(
            ...
            stability_verdict=verdict,  # type: ignore[arg-type]
