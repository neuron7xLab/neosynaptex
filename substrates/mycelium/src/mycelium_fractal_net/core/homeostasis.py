...
class HomeostasisReport:
    ...
    def __str__(self) -> str:
        ...
        lines = [
            ...
            f"║  σ: {self.initial_entropy_production:.6f} → {self.final_entropy_production:.6f}".ljust(
                w + 1
            )
            + "║",
            f"║  Heals: {self.heals_attempted} attempted, {self.heals_succeeded} succeeded".ljust(
                w + 1
            )
            + "║",
...
class HomeostasisLoop:
    ...
    def run(self, seq: Any) -> HomeostasisReport:
        ...
        for iteration in range(self.max_iterations):
            ...
            if verdict.passed:
                ...
                try:
                    ...
                    if heal_result.healed and heal_result.seq_after is not None:  # type: ignore[attr-defined]
                        current_seq = heal_result.seq_after  # type: ignore[attr-defined]
        ...
        if not report.converged:
            report.final_entropy_production = (
                report.entropy_trajectory[-1] if report.entropy_trajectory else 0.0
            )
