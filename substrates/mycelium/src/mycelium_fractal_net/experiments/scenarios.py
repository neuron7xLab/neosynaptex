...
SCENARIO_HEALTHY = ScenarioConfig(
    name="healthy",
    sim_params={"grid_size": 32, "alpha": 0.18, "spike_probability": 0.25, "turing_enabled": True},
...
SCENARIO_PATHOLOGICAL = ScenarioConfig(
    name="pathological",
    sim_params={"grid_size": 32, "alpha": 0.05, "spike_probability": 0.01, "turing_enabled": False},
...
ABLATION_CONFIGS: dict[str, dict[str, object]] = {
    "baseline": {"grid_size": 32, "alpha": 0.18, "spike_probability": 0.25, "turing_enabled": True},
    "ablate_topology": {
        "grid_size": 32,
        "alpha": 0.18,
        "spike_probability": 0.25,
        "turing_enabled": False,
    },  # kill Turing patterns → topology
    "ablate_fractal": {
        "grid_size": 8,
        "alpha": 0.18,
        "spike_probability": 0.25,
        "turing_enabled": True,
    },  # small grid → no fractal structure
    "ablate_dynamics": {
        "grid_size": 32,
        "alpha": 0.18,
        "spike_probability": 0.0,
        "turing_enabled": True,
    },  # no spikes → suppressed dynamics
}
