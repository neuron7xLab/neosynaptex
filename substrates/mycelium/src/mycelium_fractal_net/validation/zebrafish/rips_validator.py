...
class RipsValidationReport:
    ...
    pre_validated: dict = field(default_factory=PRE_VALIDATED.copy)
...
def _rips_lifetimes(points: np.ndarray, max_eps: float) -> np.ndarray:
    ...
    lt = [de - b for d, (b, de) in st.persistence() if d == 0 and de != float("inf")]
...
class RipsMHLComputer:
    ...
    def compute(
        ...
        N = len(points)
        max_eps = self.max_epsilon or self._auto_eps(points)
    ...
    def compute_series(
        ...
        for i, pts in enumerate(point_clouds):
            ...
            max_eps = self.max_epsilon or self._auto_eps(pts)
...
class RipsControlGenerator:
    ...
    def full_permutation_series(self, clouds: list[np.ndarray]) -> list[np.ndarray]:
...
class RipsValidator:
    ...
    def __init__(
        ...
        self.computer = RipsMHLComputer(max_epsilon=max_epsilon, max_points=max_points)
    ...
    def validate(
        ...
        wt_result = self.computer.compute_series(wt_clouds, "wild_type", label_real)
        ctrl_result = self.computer.compute_series(ctrl_clouds, "random_control", label_real)
        ...
        mut_result = None
        if mut_clouds:
            mut_result = self.computer.compute_series(mut_clouds, "mutant", label_real)
            if self.verbose:
                print(f"  Mut:  MHL={mut_result.median_lifetime:.3f}")

        sep = (
            ctrl_result.median_lifetime / wt_result.median_lifetime
            if wt_result.median_lifetime > 0 and wt_result.median_lifetime != float("inf")
            else 0.0
        )
    ...
    def from_mat_directory(
        ...
        def _extract_clouds(
            ...
            valid = [
                (ck, nk)
                for ck, nk in zip(cell_keys, num_keys, strict=False)
