...
class TDAFrameExtractor:
    ...
    def extract_series(
        ...
        for i, field_arr in enumerate(density_fields):
            ...
            sig = compute_tda(field_arr, min_persistence_frac=self.min_persistence_frac)
...
class CalibratedGammaComputer:
    ...
    def compute(
        ...
        if len(frames) < 5:
            notes.append(f"# APPROXIMATION: only {len(frames)} frames, need >= 5")
        ...
        if n_pts < 3:
            ...
            if len(np.unique(b0_series)) >= 3:
                with warnings.catch_warnings():
                    ...
                    gamma_result = _compute_gamma_robust(b0_series, pe0_series, self.n_bootstrap)
                notes.append("# APPROXIMATION: gamma from direct pe0(beta0), not log-log")
            else:
...
class TDACalibratedValidator:
    ...
    def validate(
        ...
        """Full validation."""
        print(f"[TDA-CALIBRATED] Processing {len(wt_data)} WT + {len(mut_data)} Mutant frames...")
        ...
        tr_density = [self.kde.compute_density_field(d) for d in tr_data] if tr_data else None
        ...
        tr_frames = (
            self.tda.extract_series(tr_density, verbose=self.verbose) if tr_density else None
        )

        wt_result = self.gam.compute(wt_frames, "wild_type", label_real)
        mut_result = self.gam.compute(mut_frames, "mutant", label_real)
        tr_result = self.gam.compute(tr_frames, "transition", label_real) if tr_frames else None

        wt_ok = wt_result.valid and wt_result.gamma_near_1 and wt_result.p_value < 0.05
        mut_ok = not mut_result.gamma_near_1 or mut_result.r_squared < 0.1
    ...
    def from_mat_directory(
        ...
        adapter = ZebrafishFieldAdapter(AdapterConfig(target_grid_size=grid_size))
        ...
        tr_file = next(
            (f for f in mat_files if "pfef" in f.name and f != mut_file),
            None,
        )
        ...
        return self.validate(wt_fields, mut_fields, tr_fields, label_real=label_real)
