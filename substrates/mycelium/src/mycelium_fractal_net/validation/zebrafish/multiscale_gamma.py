...
class MultiScaleGammaComputer:
    ...
    def compute_single(
        ...
        with warnings.catch_warnings():
            ...
            r = _compute_gamma_robust(np.array(log_scales), np.array(log_b0), self.n_bootstrap)
    ...
    def compute_series(
        ...
        if not fields:
            return self._insufficient(phenotype, label_real, ["NO_FIELDS"], (), (), 0.0)
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            r = _compute_gamma_robust(np.array(all_log_s), np.array(all_log_b), self.n_bootstrap)

        gamma = r["gamma"]
        r2 = r["r2"]
        valid = r2 >= self.min_r2 and n_pts >= min_needed

        notes.append(f"# Series: {len(fields)} frames x {len(self.scales)} scales = {n_pts} points")
...
class MultiScaleValidator:
    ...
    def __init__(
        ...
        self.computer = MultiScaleGammaComputer(scales=scales, n_bootstrap=n_bootstrap)
    ...
    def validate(
        ...
        print(f"\n[MULTI-SCALE gamma] {len(wt_fields)} WT frames, scales={list(DEFAULT_SCALES)}")

        control_fields = self.control_gen.spatial_shuffle(wt_fields)

        wt_result = self.computer.compute_series(wt_fields, "wild_type", label_real)
        ctrl_result = self.computer.compute_series(control_fields, "random_control", label_real)

        mut_result = None
        if mut_fields:
            mut_result = self.computer.compute_series(mut_fields, "mutant", label_real)
        ...
        if wt_result.valid and ctrl_result.valid and wt_org and ctrl_not and sep_ok:
            verdict = "SUPPORTED"
        elif not wt_result.valid or not ctrl_result.valid or (wt_org and not sep_ok):
