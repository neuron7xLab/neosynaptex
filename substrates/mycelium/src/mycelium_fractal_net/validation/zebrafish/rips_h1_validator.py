...
class H1ValidationReport:
    ...
    def summary(self) -> str:
        lines = [
            ...
            "Metric: H1 Median Lifetime (gudhi.RipsComplex dim=2)",
        ...
        lines += [
            ...
            "Pre-validated: stripe=0.541+/-0.128, noise=1.082+/-0.151 [ratio=2.00x, Cohen d=2.73]",
...
class RipsH1Computer:
    ...
    def _h1_lifetimes(self, pts: np.ndarray) -> np.ndarray:
        ...
        lt = [de - b for d, (b, de) in st.persistence() if d == 1 and de != float("inf")]
...
class RipsH1Validator:
    ...
    def validate(
        ...
        mut_r = (
            self.computer.compute_series(mut_clouds, "mutant", label_real) if mut_clouds else None
        )
        ...
        sep = ctrl_r.h1_median / wt_r.h1_median if 0 < wt_r.h1_median < float("inf") else 0.0
        ...
        if not valid:
            ...
            g6 = label_real
            note = (
                "G6 CLOSED on real data." if label_real else "G6 pending: re-run with --real-data"
            )
