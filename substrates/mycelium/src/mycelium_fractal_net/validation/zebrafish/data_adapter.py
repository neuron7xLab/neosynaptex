...
class ZebrafishFieldAdapter:
    ...
    def from_arrays(
        ...
        for i, arr in enumerate(arrays):
            ...
            if i == 0:
                ...
                history = np.concatenate([prev, processed[np.newaxis, :, :]], axis=0)
    ...
    def from_npz(self, path: Path, phenotype: str = "wild_type") -> list[FieldSequence]:
        ...
        if "images" in data:
            ...
            raise KeyError(
                f"Expected 'images' or 'image' key in {path}. Found: {list(data.keys())}"
            )
    ...
    def from_mat(
        ...
        for i, seq in enumerate(sequences):
            ...
            meta["ref"] = (
                "McGuirl et al. (2020) PNAS 117(10):5217-5224. DOI: 10.1073/pnas.1917763117"
            )
            patched.append(
                FieldSequence(
                    field=seq.field,
    ...
    def from_mat_composite(
        ...
        if not valid_pairs:
            raise ValueError(
                f"No valid cell types in {path.name}. Tried: {list(zip(cell_keys, num_keys, strict=False))}"
            )
    ...
    def _process_field(self, arr: np.ndarray, idx: int) -> np.ndarray:
        ...
        if arr.ndim != 2:
            raise ValueError(f"Expected 2D field at timepoint {idx}, got shape {arr.shape}")
