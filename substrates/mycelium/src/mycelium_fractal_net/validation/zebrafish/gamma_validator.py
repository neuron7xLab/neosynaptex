...
class ZebrafishGammaValidator:
    ...
    def validate(
        ...
        if transition_sequences:
            trans_result = self._validate_phenotype(transition_sequences, "transition", label_real)
        ...
        mut_differs = (
            not mut_result.hypothesis_1_0 or mut_result.r_squared < self.min_r2_for_power_law
        )
    ...
    def _validate_phenotype(
        ...
        if len(sequences) < 5:
            notes.append(f"# APPROXIMATION: only {len(sequences)} sequences, need >= 5")
        ...
        if len(log_x) < 3:
            ...
            if len(entropies) >= 3 and len(set(complexities)) >= 2:
                ...
                gamma_result = _compute_gamma_robust(x_arr, y_arr, self.n_bootstrap, rng_seed=42)
                notes.append("# APPROXIMATION: fallback to direct polyfit (not log-log)")
            else:
                return self._insufficient_result(
                    phenotype,
                    label_real,
