...
class ZebrafishReportExporter:
    def to_json(self, report: ZebrafishValidationReport, path: Path | None = None) -> str:
    ...
    def to_markdown(self, report: ZebrafishValidationReport, path: Path | None = None) -> str:
        """Generate Markdown report for manuscript."""
        wt = report.wild_type
        mut = report.mutant

        evidence_label = "SYNTHETIC PROXY" if not report.label_real else "REAL DATA"
