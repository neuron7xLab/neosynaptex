...
class MFNFeatureExtractor:
    ...
    def extract_causal_features(
        ...
        features["stage_activation_entropy"] = float(-np.sum(a_norm * np.log(a_norm + 1e-12)))
    ...
    def extract_all(
        ...
        thermo = (
            self.extract_thermodynamic_features(thermo_report) if thermo_report is not None else {}
        )
        topo = self.extract_topological_features(seq)
        fractal = self.extract_fractal_features(seq)
        causal = self.extract_causal_features(causal_result) if causal_result is not None else {}
