...
def phi_sovereign(
    ...
    theta_ok = bool(np.all(selection.theta_vector >= 0.1) and np.all(selection.theta_vector <= 0.9))
...
def compute_reality(
    ...
    # phi — SovereignGate
    sovereign_passed, sovereign_lenses, sovereign_confidence = phi_sovereign(compression, selection)

    # Reality classification
    label, confidence = classify_reality(compression, selection, sovereign_passed)
...
def resolve_reality(
    ...
    scores = [
        label_score.get(f.reality_label, 0.5) + (1.0 - f.reality_confidence) for f in alternatives
    ]

    # CCP states for criticality-based selection
    ccp_states = [{"D_f": f.compression.D_f, "R": f.compression.R} for f in alternatives]
...
def _tests_classify(test_fn) -> None:
    ...
    def _test_classify_cognitive():
        c = CompressionResult(
            D_f=1.7, phi_proxy=0.1, R=0.6, anomaly_score=0.2, regime="stable", cognitive=True
        )
        s = SelectionResult(
            gnc_regime="optimal",
            coherence=0.8,
            dominant_axis="Glutamate",
            meso_strategy="EXPLOIT",
            theta_vector=np.full(9, 0.5),
            modulated_anomaly=0.2,
            program_spine={},
    ...
    def _test_classify_pathological():
        c = CompressionResult(
            ...
            R=0.1,
            anomaly_score=0.9,
            regime="pathological_noise",
            cognitive=False,
        )
        s = SelectionResult(
            gnc_regime="dysregulated",
            coherence=0.2,
            dominant_axis="Opioid",
            meso_strategy="RESET",
            theta_vector=np.full(9, 0.5),
            modulated_anomaly=0.9,
            program_spine={},
