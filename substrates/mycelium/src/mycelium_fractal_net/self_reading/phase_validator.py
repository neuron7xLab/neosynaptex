...
class PhaseValidator:
    ...
    def validate(
        ...
        if thermo_report is not None:
            ...
            free_energy = (
                thermo_report.energy_trajectory[-1] if thermo_report.energy_trajectory else 0.0
            )
        ...
        transition_risk = (
            float(np.mean([1.0 for p in recent if p != MFNPhase.OPERATIONAL])) if recent else 0.0
        )
