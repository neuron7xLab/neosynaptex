...
def _cubical_tda(image_2d: np.ndarray) -> tuple[float, int]:
    ...
    try:
        ...
        h0 = [(b, d) for dim, (b, d) in raw_pairs if dim == 0 and d != float("inf")]
    except ImportError:
...
class GammaFieldProbe:
    ...
    def measure(self, fields: NDArray, substrate: str = "field") -> GammaFieldReport:
        """
    ...
    def measure_control(self, fields: NDArray, substrate: str = "shuffled") -> GammaFieldReport:
        """
    ...
    def run_on_engine(
        ...
        history, _metrics = engine.simulate(steps=steps, turing_enabled=True, return_history=True)
        # history shape: (steps, grid_size, grid_size)
        real = self.measure(history, substrate=substrate_label)
        control = self.measure_control(history, substrate=f"{substrate_label}_shuffled")
        return real, control

    # -- internals -----------------------------------------------------------

    def _compute_tda_series(self, fields: NDArray) -> tuple[np.ndarray, np.ndarray]:
