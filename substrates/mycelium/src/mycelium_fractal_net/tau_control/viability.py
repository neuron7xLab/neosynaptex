...
class PolynomialDynamicsApproximator:
    ...
    def fit(
        ...
        X_t = np.array([np.asarray(s.state_vector, dtype=np.float64) for s in snapshots[:-1]])
        X_t1 = np.array([np.asarray(s.state_vector, dtype=np.float64) for s in snapshots[1:]])
...
class PODProjector:
    ...
    def fit(self, snapshots: list[MFNSnapshot]) -> PODProjector:
        ...
        X = np.array([np.asarray(s.state_vector, dtype=np.float64) for s in snapshots])  # (n, d)
        self._d = X.shape[1]
...
class GalerkinODE:
    ...
    def fit(
        ...
        coeffs = np.array(
            [pod.project(np.asarray(s.state_vector, dtype=np.float64)) for s in snapshots]
        )  # (n, r)
        a_t = coeffs[:-1]  # (n-1, r)
        a_t1 = coeffs[1:]  # (n-1, r)
...
class CertifiedViabilityV4:
    ...
    def fit_modal_ellipsoid(
        ...
        modal_coords = np.array(
            [self.pod.project(np.asarray(s.state_vector, dtype=np.float64)) for s in snapshots]
        )
