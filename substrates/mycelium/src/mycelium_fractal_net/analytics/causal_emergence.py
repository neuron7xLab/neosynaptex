...
def discretize_field_pca(
    ...
    if method == "kmeans":
        try:
            ...
            return km.fit_predict(proj).astype(int)  # type: ignore[no-any-return]
        except ImportError:
            method = "quantile"
    ...
    bins[0] -= 1e-10
    bins[-1] += 1e-10
    return np.clip(np.digitize(pc1, bins[1:-1]), 0, n_macro_states - 1).astype(int)  # type: ignore[no-any-return]
