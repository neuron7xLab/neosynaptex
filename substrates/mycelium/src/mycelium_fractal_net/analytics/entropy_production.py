...
class EntropyProductionResult:
    ...
    regime: (
        str  # "equilibrium", "near_equilibrium", "far_from_equilibrium", "dissipative_structure"
    )

    def summary(self) -> str:
        return (
            f"[ENTROPY] σ={self.sigma:.6f} ({self.regime}) eq_dist={self.equilibrium_distance:.3f}"
        )
...
def entropy_production_trajectory(
    ...
    return [
        compute_entropy_production(history[t], alpha) for t in range(0, history.shape[0], stride)
    ]
