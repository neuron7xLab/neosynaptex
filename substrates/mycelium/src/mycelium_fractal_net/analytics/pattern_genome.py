...
class PatternGenome:
    ...
    def fingerprint(self) -> np.ndarray:
        ...
        b0 = np.interp(
            np.linspace(0, 1, 10), np.linspace(0, 1, len(self.betti_curve_0)), self.betti_curve_0
        )
        b1 = np.interp(
            np.linspace(0, 1, 10), np.linspace(0, 1, len(self.betti_curve_1)), self.betti_curve_1
        )
        ...
        ec = np.interp(
            np.linspace(0, 1, 20), np.linspace(0, 1, len(self.euler_curve)), self.euler_curve
        )
