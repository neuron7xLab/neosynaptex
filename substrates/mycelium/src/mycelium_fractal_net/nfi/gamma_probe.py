...
class GammaEmergenceProbe:
    ...
    def analyze(self, contracts: list[NFIStateContract]) -> GammaEmergenceReport:
        ...
        d_boxes = [
            c.mfn_snapshot.d_box if c.mfn_snapshot.d_box is not None else 1.0 for c in contracts
        ]
    ...
    def _theil_sen_bootstrap(self, x: np.ndarray, y: np.ndarray) -> dict:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", np.exceptions.RankWarning)
            null = [
                float(np.polyfit(x, rng.permutation(y), 1)[0]) for _ in range(self._n_bootstrap)
            ]
        p_value = max(float(np.mean(np.abs(null) >= abs(gamma_ts))), 1.0 / self._n_bootstrap)

        ci_excludes_zero = not (ci_lo <= 0.0 <= ci_hi)
        gate_pass = ci_excludes_zero and p_value < 0.05 and abs(gamma_ts) > 0.3 and r2_ts > 0.3
