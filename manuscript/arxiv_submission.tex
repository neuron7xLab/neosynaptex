...
\fancyhead[C]{\footnotesize\itshape\color{dimgray} $\gamma$-scaling at the edge of metastability}
...
% TITLE
% ====================================================================
\title{\LARGE\bfseries $\gamma$-scaling at the edge of metastability: an analytical anchor for Kuramoto dense graphs and an empirical cross-substrate program}
...
\begin{abstract}
\noindent We reframe the onset of coherence in coupled-oscillator systems as a cost--complexity scaling law $K(C) \sim C^{-\gamma}$, with $K := J_c$ (critical coupling) and $C := \lambda_1(A)$ (largest adjacency eigenvalue).

\medskip
\noindent\textbf{Analytical anchor.}
We prove $\gamma = 1$ for the Kuramoto model on dense symmetric graphs and test the broader cross-substrate conjecture empirically. Lemma~1 (§2) reformulates the Restrepo--Ott--Hunt spectral-onset formula in these operational variables, giving $J_c = (2/(\pi g(0)))\cdot \lambda_1(A)^{-1}$ under assumptions (A0)--(A3). Numerical verification on complete graphs $K_N$ with Lorentzian frequencies $\Delta = 0.5$ at $N \in \{30, 100, 300, 1000, 3000\}$, 48 samples per $N$, master seed~$7$, yields $\hat\gamma = 0.9923$ with 95\% CI $[0.9811,\; 1.0032]$ containing the analytical value.

\medskip
\noindent\textbf{Empirical cross-substrate program.}
We then measure $\gamma$ on three biological substrates under separate preregistrations and surrogate-null controls: zebrafish morphogenesis ($\gamma = 1.055$, $n = 47$), human heart-rate variability ($\gamma = 0.885$, PhysioNet NSR2DB), and human EEG during motor imagery ($\gamma \approx 1.07$, $n = 20$). Three simulation substrates anchor the methodology: Gray--Scott reaction--diffusion ($\gamma = 0.938$), Kuramoto oscillators ($\gamma = 0.980$), and BN-Syn spiking criticality ($\gamma \approx 0.49$ at $N = 200$, expected finite-size deviation). Cross-substrate mean from the three evidential biological substrates: $\bar\gamma = 1.003 \pm 0.083$, 95\% CI containing unity; all pass surrogate tests ($p < 0.05$); three negative controls (white noise, random walk, supercritical) show $\gamma$ clearly separated from unity.

\medskip
\noindent\textbf{Scope.} Analytical proof is currently available only for the Kuramoto dense-graph case; universality across other substrates remains an empirical conjecture.
\end{abstract}
...
K \sim C^{-\gamma}
\end{equation}
where $C$ is a measure of the system's structural or information complexity and $K$ is the energetic or computational cost per unit of complexity. We present evidence that $\gamma \approx 1.0$ across five independent substrates, suggesting it may represent a signature of metastable computation.
...
\paragraph{Contribution.} This work contributes (i) an operational reframing of critical onset as a cost--complexity scaling law $K(C) \sim C^{-\gamma}$; (ii) a rigorous anchor case, Lemma~1 (§2), proving $\gamma = 1$ for the Kuramoto model on dense symmetric graphs with numerical verification $\hat\gamma = 0.9923$ (95\% CI $[0.9811, 1.0032]$); and (iii) an empirical cross-substrate program testing whether $\gamma \approx 1$ appears beyond Kuramoto. Analytical proof is currently available only for the Kuramoto dense-graph case; universality across other substrates remains an empirical conjecture. A full statement of what this paper does and does not claim is in \texttt{docs/CONTRIBUTION\_STATEMENT.md}.
...
\subsection{Cross-substrate convergence}
...
We propose that $\gamma \approx 1.0$ is a scaling signature of metastability in substrates that already operate in a moderate-topological-variability regime --- a \textbf{substrate-specific candidate condition} for coherent computation at the edge of chaos, not a substrate-independent invariant. Substrate-independence was empirically contradicted by the 2026-04-14 HRV n=5 pilot ($\gamma$ mean $0.50 \pm 0.44$ across subjects). If confirmed by independent replication within each substrate, this finding may still have implications for AI alignment (productive coupling has a measurable signature), cognitive enhancement ($\gamma$ as a real-time, per-substrate-calibrated diagnostic), and the physics of intelligence (metastability as a per-substrate regime marker rather than a cross-substrate invariant).
