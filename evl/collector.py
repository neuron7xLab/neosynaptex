...
def gen_pattern(difficulty: int = 2) -> Task:
    ...
    for i in range(len(haystack) - needle_len + 1):
        if haystack[i : i + needle_len] == needle:
            count += 1
    return Task(
        _uid(), "pattern", f"Count occurrences of '{needle}' in: {haystack}", str(count), difficulty
    )
...
GENERATORS = [gen_arithmetic, gen_modular, gen_sequence, gen_bitwise, gen_pattern, gen_logic]
...
class SessionRunner:
    ...
    def run(self):
        ...
        manifest = {
            ...
            "perturbation_schedule": self.perturbation.schedule
        ...
        print(f"\n{'=' * 60}")
        print("  NFI v2.1 — Channel B: Decision Latency Collector")
        ...
        try:
            while True:
                ...
                self.task_count += 1
                print(
                    f"  [{phase_display}] Task #{self.task_count} "
